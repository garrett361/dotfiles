...
2. Launch 2-3 reviewer subagents in parallel, each with a distinct lens: bugs,
   faithfulness and completeness to the plan, convention (`CLAUDE.md` /
   `AGENTS.md` and local style). Inline the plan text and the scoped diff from
   step 1 into each prompt so all reviewers share one scope; grant file read
   access. Tell each to return only critical and borderline findings (drop nits)
